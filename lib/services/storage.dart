...
class Storage {
        ...
        .snapshots().listen((data) {
          print(data.documents.length);
           for(int i=0 ; i<data.documents.length ;i++ ) {
             Cloudfile p = new Cloudfile(File_name: data.documents[i].data['File_Name']
  ...
  Future createMetadata(String uid){
    StorageReference privateRefMetadata = FirebaseStorage.instance.ref().child("Private_Files/" + uid +"/metadata");
    ...
    if( Token.length != 0 ) {
      for( int i=0;i<Token.length ; i++ ){
        ...
        if(Token[i] != null || Token[i] != uid+"\n"){
          Cloudfile p = new Cloudfile();
          await privateCollection.where('File_Name', isEqualTo: Token[i])
  ...
  Future uploadFileToPrivate(String File_Name, String path, String uid) async {
    ...
    StorageReference privateUploadRef = reference.child("Private_Files/" + uid  + "/" + File_Name + stamp);
    StorageUploadTask task1 = await privateRefMetadata.putData(newbytes);   // adding name to metadata.txt
    StorageUploadTask task2 = await privateUploadRef.putFile(File(path));   // adding file to sto
  }

  Future<dynamic> downloadPrivateFileWithUrl(String uri, String uid) async {
    StorageReference publicDownloadRef = FirebaseStorage.instance.ref().child("Private_Files/" + uid + "/" + uri);
  ...
  Future<dynamic> fetchFileFromKey(String key) async {
    ...
    var url;
    print(uid);
    await privateCollection.where("Key", isEqualTo: key)
        .snapshots().listen((data) {
          print("Key is valid");
          print(data.documents.length);
          f = data.documents[0].data['File'];
          print(f);
          StorageReference publicDownloadRef = reference.child("Private_Files/" + uid + "/" + f);
